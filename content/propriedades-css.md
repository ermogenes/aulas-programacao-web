...
- `font-family` permite alterar a face da fonte a ser exibida (tipo). Recebe uma lista de fontes e utiliza a primeira que estiver disponível. Use somentes fontes que existam no dispositivo do usuário (veja quais fontes são seguras [aqui](https://www.cssfontstack.com/)), ou use uma Web Font (como as fontes do [Google Fonts](#Usando-fontes-do-Google)).
  ...
  - `font-family: Arial, san-serif;` - use um nome genérico como última alternativa:
...
- `font-weight` permite alterar o peso da fonte. Comumente usado para colocar o texto em negrito. Algumas Web Fonts permitem diversos pesos, numerados entre 100 a 900 (400 = normal, 700 = negrito).
  ...
  - `font-weight: lighter;` - fonte um tamanho menor que a herdada (entre 700, 400 ou 100).
  - `font-weight: bolder;` - fonte um tamanho maior que a herdada (entre 400, 700 ou 900).
...
Adicionando um quarto valor ao hexadecimal (`00` a `FF`) ou à função `rgb` (`0.` a `1.`, ou um `%`), mudamos o canala _alpha_, que nos indica a opacidade do elemento:

- `#0000ff88` ou `rgb(0, 0, 255, 0.5)` - indica um azul 50% transparente.
- `#0000ff00` ou `rgb(0, 0, 255, 0)` -indica um azul 100% transparente.
- `#0000ffff` ou `rgb(0, 0, 255, 1)` -indica um azul 0% transparente (opaco).
